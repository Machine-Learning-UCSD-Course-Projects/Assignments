...
\begin{abstract}
In this project we try to find topics associated with a corpus of documents using Latent Dirichlet Allocaion(LDA). We use two data sets 1)Classic400, which was given as a part of the project and 2) Hookah dataset, which is a collection of tweets on Hookah. In the first dataset we find 3 dominant topics associated with classic400 namely Aeronautics, Biology and scientific research. In the seond dataset our objective was to find the sentiment people associate with Hookas. We found positive feelings like party, friends etc. associated with Hookah. We were able to fit the labels of classic400 to about 93%.
\end{abstract}
...
\caption{Distribution of documents for classic400 dataset when $\alpha = 10$}
...
\begin{tabular}{|c|c|c|c|}
\hline 
Topics in training label & Calculated Topics & Number of document for training label & Number of documents for calculated training label \\ 
\hline 
